...
// 생활권/역세권/이용 장소 데이터를 본문·FAQ·체크리스트에 반영합니다.
...
// 공통 안내 배너 (개인정보 / 불법·선정적 서비스 불가)
const POLICY_NOTICE = `<div class="notice">
  간다GO는 예약 확인과 연락에 필요한 <a href="/seoul-service/check/privacy/">최소 개인정보</a>만 확인하며,
...
// Who/How/Why 기본 블록
const WHW = whwBlock({
  who: '이 콘텐츠는 서울 지역 방문형 웰니스 서비스 이용 전, 사용자가 숙소 유형·건물 출입·생활권·예약 조건을 확인할 수 있도록 작성되었습니다.',
  how: '서울시 공식 자치구·행정동 자료와 실제 예약 전 확인 항목, 개인정보 처리 기준을 바탕으로 작성하며, AI 보조 도구를 사용해도 최종 문구는 사람이 검수합니다.',
  why: '검색 순위 조작이 아니라, 서울에서 호텔·오피스텔·아파트·업무지구 이용 전 필요한 확인사항을 이해하기 쉽게 안내하기 위해서입니다.',
});
...
// 이용 장소별 차이 표준 설명
const USE_DIFF = `<h2>이용 장소별로 확인 사항이 다릅니다</h2>
...
예약 전에는 지역명보다 먼저 <a href="/seoul-service/use/hotel/">이용 장소</a>를 확인하는 것이 정확합니다.</p>
<ul>
  <li><a href="/seoul-service/use/hotel/">호텔·숙소</a> — 객실 출입 가능 여부, 프런트 확인, 예약자명, 야간 출입</li>
  <li><a href="/seoul-service/use/officetel/">오피스텔</a> — 공동현관, 엘리베이터, 경비실, 관리 규정, 방문 가능 시간</li>
  <li><a href="/seoul-service/use/apartment-home/">아파트·자택</a> — 동·호수, 공동현관, 경비실, 주차</li>
  <li><a href="/seoul-service/use/business-district/">업무지구</a> — 퇴근 후 예약, 차량 이동, 건물 출입, 야간 시간대</li>
</ul>`;

// ---------------------------------------------------------------------------
// 허브(이용 상황별) 페이지 본문
// ---------------------------------------------------------------------------
export function hubBody(hub, relatedLinks) {
  ...
  <div class="container article">
    <p class="eyebrow">이용 상황별 안내</p>
    <h1>${hub.h1}</h1>
    <p class="lede">서울에서 ${hub.focus} 전 확인해야 할 주소·건물 출입·예약 가능 시간 기준을 정리했습니다. ${site.brand}는 실제 예약 조건을 확인한 뒤 방문 가능 여부를 안내합니다.</p>
    ${figure(hub.imgAlt)}

    <h2>${hub.focus} 개요</h2>
    <p>${hub.angle} 서울은 25개 자치구와 427개 행정동으로 구성되어 있어 지역명만으로는 이용 조건을 판단하기 어렵습니다. 그래서 ${hub.focus}처럼 이용 상황을 먼저 확인하는 것이 예약을 더 정확하게 만듭니다.</p>

    <h2>주요 이용 생활권</h2>
    <p>${hub.focus}와 관련해 문의가 많은 서울 생활권은 다음과 같습니다. 같은 생활권이라도 건물·숙소 유형에 따라 확인 사항이 달라집니다.</p>
    <ul>${areas}</ul>

    ${USE_DIFF}

    <h2>건물 출입 기준</h2>
    <p>공동현관 인증, 엘리베이터 카드, 경비실 방문자 등록, 프런트 확인 방식은 건물마다 다릅니다. 도착 후 지연을 줄이려면 예약 전에 출입 방식을 미리 확인하는 것이 좋습니다. 자세한 내용은 <a href="/seoul-service/check/building-access/">건물 출입 방식</a>과 <a href="/seoul-service/check/apartment-access/">공동현관 확인</a> 페이지를 참고하세요.</p>
...
// 시간대별 페이지 본문
// ---------------------------------------------------------------------------
export function timeBody(t, relatedLinks) {
  const faq = [
  ...
  return {
    ...
    html: `<section class="section">
  <div class="container article">
    <p class="eyebrow">시간대별 안내</p>
    <h1>${t.h1}</h1>
    <p class="lede">${t.desc}</p>
    ${figure(`서울 ${t.name} 안내 이미지`)}
...
// 생활벨트 페이지 본문
// ---------------------------------------------------------------------------
export function beltBody(belt, relatedLinks) {
  ...
  <div class="container article">
    ...
    <h2>건물 출입 기준</h2>
    <p>${belt.name} 내 호텔·오피스텔·아파트는 공동현관 인증, 프런트 확인, 경비실 방문자 등록 방식이 서로 다릅니다. <a href="/seoul-service/check/building-access/">건물 출입 방식</a> 페이지에서 유형별 확인 사항을 참고하세요.</p>
...
// 구 페이지 본문
// ---------------------------------------------------------------------------
export function districtBody(d, beltName, relatedLinks) {
  ...
  return {
    ...
    html: `<section class="section">
  <div class="container article">
    <p class="eyebrow">서울 구별 안내</p>
    <h1>${d.h1}</h1>
    <p class="lede">${d.desc}</p>
    ${figure(`${d.name} 이용 안내 이미지`)}

    <h2>${d.name} 개요</h2>
    <p>${d.name}는 단순 지역명보다 ${d.sub.join(', ')}처럼 생활권 차이가 큽니다. 업무지구와 호텔, 오피스텔, 주거지가 섞여 있으므로 예약 전 주소, 건물 출입, 숙소 규정, 이동 기준을 함께 확인해야 합니다. ${d.name}는 <a href="/seoul-service/belt/${d.belt}/">${beltName}</a>에 속합니다.</p>
...
// 역세권 페이지 본문
// ---------------------------------------------------------------------------
export function stationBody(s, relatedLinks) {
  const faq = [
  ...
  <div class="container article">
    ...
    <h2>건물 출입 기준</h2>
    <p>${s.name} 인근 건물은 공동현관·프런트·경비실 확인 방식이 다양합니다. <a href="/seoul-service/check/building-access/">건물 출입 방식</a>과 <a href="/seoul-service/check/time/">예약 가능 시간</a> 페이지를 함께 확인하세요.</p>
...
// 예약 전 확인(체크) 페이지 본문
// ---------------------------------------------------------------------------
export function checkBody(c, relatedLinks) {
  const faq = [
