# 간다GO — 서울·경기 출장마사지 안내 사이트

이용 장소·생활권 중심으로 설계한 다지역 출장마사지 안내 정적 사이트입니다.
행정구역 나열형이 아니라 **호텔·오피스텔·아파트·업무지구·관광 숙소·야간·역세권** 등
이용 목적별 허브 구조로 구성해, 도어웨이/중복 페이지 스팸 리스크를 낮췄습니다.
현재 **서울·경기** 2개 지역을 동일 엔진으로 생성합니다.
...
```bash
node build.js      # → dist/ 에 전체 사이트 생성 (148 indexable 페이지)
```
...
### 페이지 구성 (148 indexable)

| 그룹 | 경로 | 서울 | 경기 |
|---|---|---|---|
| 지역 허브 메인 | `/{region}-service/` | 1 | 1 |
| 이용 상황별 허브 | `.../use/*` | 8 | 8 |
| 시간대별 안내 | `.../time/*` | 5 | 5 |
| 8대 생활벨트 | `.../belt/*` | 8 | 8 |
| 구/시 안내 | `.../district/*` | 22 | 22 |
| 핵심 역세권 | `.../station/*` | 14 | 14 |
| 예약 전 확인 | `.../check/*` | 12 | 12 |

전역: 홈 `/`, 운영/정책/문의 6, 사이트맵 `/sitemap/`.
