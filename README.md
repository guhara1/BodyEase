...
이용 장소·생활권 중심으로 설계한 서울 출장마사지 안내 정적 사이트입니다 (지침서 2번째 버전).
행정구역 나열형이 아니라 **호텔·오피스텔·아파트·업무지구·관광 숙소·야간·역세권** 등
이용 목적별 허브 구조로 구성해, 도어웨이/중복 페이지 스팸 리스크를 낮췄습니다.

빌드 대상 지역은 `src/data/regions/index.js`의 `regions` 배열로 관리합니다.
현재는 지침서 범위에 맞춰 **서울만** 빌드하며, 다른 지역 데이터 파일(경기·인천·부산 등
25개)은 리포지토리에 보관되어 있어 배열에 추가하면 즉시 다지역으로 확장됩니다.
...
```bash
node build.js      # → dist/ 에 전체 사이트 생성 (91 indexable 페이지)
```
...
**전 페이지 index 정책**: noindex를 사용하지 않습니다. 대신 모든 페이지 유형의
본문을 2,000자 이상(생활권별 상세 안내, 확인 순서, 시간대별 참고 등 데이터 기반
고유 섹션)으로 구성해 얇은 페이지 문제를 콘텐츠로 해소했습니다. `tier`는
지역 메인의 노출 우선순위로만 사용됩니다.
...
- **중복/얇은 페이지 방지** — 출구별·노선별·번호동 개별 페이지를 만들지 않음.
  모든 페이지 본문 2,000자 이상 + 생활권 데이터 기반 고유 섹션으로 전 페이지 index.
