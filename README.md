...
이용 장소·생활권 중심으로 설계한 서울 출장마사지 안내 정적 사이트입니다.
...
```bash
node build.js      # → dist/ 에 전체 사이트 생성 (78 indexable 페이지)
```
...
│  └─ pages.js    # 허브/시간대/생활벨트/구/역세권/예약전확인/정책 데이터
├─ lib/
│  ├─ templates.js # <head>·헤더·푸터·JSON-LD 스키마 빌더
│  └─ content.js   # 페이지 본문(아티클) 빌더
...
| 그룹 | 경로 | 수 |
|---|---|---|
| 홈 / 서울 허브 메인 | `/`, `/seoul-service/` | 2 |
| 이용 상황별 허브 | `/seoul-service/use/*` | 8 |
| 시간대별 안내 | `/seoul-service/time/*` | 5 |
| 서울 8대 생활벨트 | `/seoul-service/belt/*` | 8 |
| 구별 안내 (1·2차 index) | `/seoul-service/district/*` | 22 |
| 핵심 역세권 | `/seoul-service/station/*` | 14 |
| 예약 전 확인 | `/seoul-service/check/*` | 12 |
| 운영/정책/문의 | `/about/`, `/privacy-policy/` 등 | 6 |
| 사이트맵 | `/sitemap/` | 1 |
